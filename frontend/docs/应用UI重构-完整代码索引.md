...
- 路由：`/login` → Login；`/` → 重定向到 `defaultRoute`（已登录 `/admin/accounts`，未登录 `/login`）；`/admin/*` 走 **ProtectedRoute** + **AdminLayout**，子路由 dashboard、accounts、conversations、profile。
- 全局 UI：已登录时渲染 **DesktopTitleBar**、**UpdatePromptModal**；始终渲染 **PrivacyModal**。
- 懒加载页面：Login、Dashboard、Chat、AccountManager、Settings、Profile、NotFound；**Suspense** fallback 为 Spin。
...
- **Sider**：LOGO（CM）、**Menu**（dashboard、accounts、conversations），选中 key 由 pathname 前三级决定。
...
- 使用 **GlobalSignalPanel compact={false}** 作为紧凑总览入口。
- 任务入口已并回概览，不再保留独立任务管理页。
...
4. **theme.css / global.css**：主题变量和品牌色统一控制后台主色、表格状态与卡片层级。

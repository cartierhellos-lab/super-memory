      ...
      <div className="cm-page-header cm-page-header--dashboard">
        ...
        <div className="cm-toolbar-group cm-toolbar-group--actions">
          {activeTab !== 'proxy-pool' && (
            <Button onClick={refreshCurrentTab}>{td('common.refresh', '刷新')}</Button>
          )}
          <Button onClick={() => setActiveTab('accounts')}>{td('accounts.inspect_inventory', '查看库存')}</Button>
          <Button onClick={() => setActiveTab('proxy-pool')}>{td('accounts.focus.review_proxy_pool', '查看代理池')}</Button>
          {canManageAgents && (
            <Button type="primary" className="cm-primary-button" onClick={() => setActiveTab('sub-accounts')}>
              {td('accounts.open_sub_accounts', '打开子账号')}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="cm-toolbar-shell">
        <div className="cm-toolbar-stats">
          {inventorySummary.map((item) => (
            <div key={item.key} className={`cm-toolbar-stat cm-toolbar-stat--${item.tone}`}>

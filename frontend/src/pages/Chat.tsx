...
const Chat: React.FC = () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          direction: "outbound",
          content: payload || (pendingImage ? "[image]" : ""),
          media_url: pendingImage?.url ?? "",
          status: "sent",
  ...
  }, [clearTranslationState, draft, pendingImage, scrollToBottom, selectedChat, sendMode, translateEnabled, translatedDraft, t]);
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (res?.success && res?.url) {
          setPendingImage({ url: res.url, name: file.name || "image" });
          message.success(t("upload.upload_success", { defaultValue: "上传成功" }));
        } else {
          message.error(t("upload.upload_failed", { defaultValue: "上传失败" }));
        }
      } catch (error) {
        console.error(error);
        message.error(t("upload.upload_failed", { defaultValue: "上传失败" }));
      ...
      <div className="cm-chat-shell">
        <div className="cm-chat-sidebar cm-chat-sidebar__panel">
          <div className="cm-chat-sidebar__header">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
              ...
              <Space size={6}>
                <Button size="small" onClick={() => setRemarkOpen(true)} disabled={!selectedChat}>
                  {t("chat.remark")}
                </Button>
        ...
        <div className="cm-chat-pane cm-chat-pane__panel" style={{ display: "flex", flexDirection: "column" }}>
          <div ref={msgListRef} className="cm-thread-stream">
            <div className="cm-thread-canvas">
              {loadingMsgs ? (
                <div className="cm-thread-empty">
                  <Spin />
                </div>
              ) : !selectedChat ? (
                <div className="cm-thread-empty">
                  <Text type="secondary">{t("chat.empty_copy")}</Text>
                </div>
              ) : (
                messages.map((msg) => {
                  const isMine = msg.direction === "outbound";
                  const showContent = Boolean(String(msg.content || "").trim()) && msg.content !== "[image]";
                  return (
                    <div key={msg.id} className={`cm-message-row${isMine ? " cm-message-row--mine" : ""}`}>
                      <div className={`cm-message-bubble${isMine ? " cm-message-bubble--mine" : ""}`}>
                        {msg.media_url ? (
                          <div className="cm-message-media">
                            <img src={msg.media_url} alt="media" loading="lazy" />
                          </div>
                        ) : null}
                        {showContent ? <div>{msg.content}</div> : null}
          ...
          <div className="cm-compose-wrap">
            <div className="cm-compose-shell">
              ...
              <div className="cm-compose-controls">
                <div className="cm-compose-controls__left">
                  <Button
                    size="small"
                  ...
                  <Select
                    ...
                    className="cm-compose-translate-select"
                ...
                <div className="cm-compose-controls__right">
                  <Button
                    size="small"
                    type={sendMode === "translated" ? "primary" : "default"}
                    onClick={() => setSendMode("translated")}
                    disabled={!translateEnabled || !translatedDraft}
                  >
                    译
                  </Button>
                  <Button
                  ...
                  <Button
                    ...
                    disabled={(!draft.trim() && !pendingImage) || sending || !selectedChat}
                  />
                </div>
              </div>

              {pendingImage ? (
                <div className="cm-compose-media">
                  <div className="cm-compose-media__thumb">
                    <img src={pendingImage.url} alt={pendingImage.name} />
                  </div>
                  <div className="cm-compose-media__meta">
                    <Text strong>{pendingImage.name}</Text>
                    <Text type="secondary">{t("chat.image_ready")}</Text>
                  </div>
                  <Button size="small" onClick={() => setPendingImage(null)}>

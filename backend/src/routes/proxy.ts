...
// Check proxy (legacy alias, same as test)
router.post('/proxies/:id/check', async (req, res) => {
    const conn = await pool.getConnection();
    try {
        const [rows]: any = await conn.query('SELECT * FROM proxies WHERE id = ?', [req.params.id]);
        if (!rows?.length) return res.status(404).json({ code: 404, error: 'Not found' });
        const result = await runSingleProxyTest(conn, rows[0], PING_URL_DEFAULT);
        res.json({ code: 0, status: result.ok ? 'Active' : 'Dead', latency: result.latency, error: result.error });
    } catch (e: any) {
        res.status(500).json({ code: 500, error: e.message });
    } finally {
        conn.release();

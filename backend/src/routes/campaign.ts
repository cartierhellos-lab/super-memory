...
// Get campaigns with status summary (for SendProgress.tsx) — 多租户：只返回当前租户
...
router.get("/user/tasks", getTasksHandler);
...
// Mock environment overview (TC-14)
router.get("/environment-overview", (_req, res) => {
  res.json({
    code: 0,
    data: {
      accountHealthRate: 98.5,
      proxySurvivalRate: 92.1,
      challengeTasks24h: 12,
      blockedTasks24h: 5
    }
  });
});
...
router.get("/user/strategy/runtime", (_req, res) => {
  res.json({
    code: 0,
    data: {
      pool: [
        {
          ...
          proxy_url: null,
          proxy_alive: 1,
          env_health_status: "Healthy",
          proxy_fail_count: 0,
          last_detection_code: "OK",
          proxy_last_checked_at: new Date().toISOString(),
          fp_ip: "1.2.3.4",
          fp_port: "8080",
          fp_country: "US",
          fp_device_model: "iPhone 13",
          fp_os: "iOS",
          fp_os_version: "15.0",
          fp_user_agent: "Mozilla/5.0..."
        }
      ]
    }
  });

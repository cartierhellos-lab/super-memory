...
router.get("/metrics", async (_req, res) => {
  const conn = await pool.getConnection();
  try {
    const [taskRows]: any = await conn.query("SELECT status, COUNT(*) as count FROM message_tasks GROUP BY status");
    const tasks: any = {};
    taskRows.forEach((r: any) => tasks[r.status] = r.count);

    const [accRows]: any = await conn.query("SELECT status, COUNT(*) as count FROM accounts GROUP BY status");
    const accounts: any = {};
    accRows.forEach((r: any) => accounts[r.status] = r.count);

    res.json({ tasks, accounts });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  } finally {
    conn.release();
...
// Mock reports (keep as mock or implement if needed)
router.get("/reports/tax", (_req, res) => {
  res.json({
    items: [
      { id: 1, taxId: 'TAX-2023-001', accountNo: 'ACC-8888', amount: 1500.00 },
      { id: 2, taxId: 'TAX-2023-002', accountNo: 'ACC-9999', amount: 2300.50 },
    ]
  });
});

router.post("/audit/privacy", (req, res) => {
  res.json({ success: true, mode: req.body.action });
});

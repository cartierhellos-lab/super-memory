...
- **proxy_audit**：`proxy_id` 必须与 `proxies.id` 类型一致（均为 **INT**）。若曾误建为 BIGINT UNSIGNED，执行 `scripts/migrate_proxy_audit_fk.sql` 修复；部署/建库后可用 `SHOW CREATE TABLE proxy_audit\G` 确认 `proxy_id` 为 `INT NULL`。
...
| 检查点 | 操作 | 期望 |
|--------|------|------|
| ① 两列类型对齐 | `SHOW CREATE TABLE proxies\G` 与 `SHOW CREATE TABLE proxy_audit\G` | `proxies.id` 为 `INT NOT NULL AUTO_INCREMENT`，`proxy_audit.proxy_id` 为 `INT NULL`（无 UNSIGNED） |
...
| ④ 数据无 INT 溢出 | `SELECT proxy_id FROM proxy_audit WHERE proxy_id > 2147483647 OR proxy_id < -2147483648 LIMIT 10;` | **无记录**（若有需先处理再 MODIFY） |
| ⑤ 迁移脚本幂等 | 在测试库执行两次 `mysql massmail < backend/scripts/migrate_proxy_audit_fk.sql` | 第二次执行不报错，外键仍存在 |

### 可选：未来 INT→BIGINT UNSIGNED 迁移

若业务 ID 量级可能超过 INT，可规划一次性迁移。需同步修改的表与列：

| 表 | 列 | 当前 | 目标 |
|----|----|------|------|
| proxies | id | INT | BIGINT UNSIGNED |
| proxy_audit | proxy_id | INT | BIGINT UNSIGNED NULL |
| account_proxy_bindings | proxy_id | INT | BIGINT UNSIGNED |
| sub_accounts | proxy_id | INT | BIGINT UNSIGNED NULL |

思路：`SET FOREIGN_KEY_CHECKS=0` → 依次 `ALTER TABLE … MODIFY … BIGINT UNSIGNED`（先 DROP 依赖此外键的约束再 MODIFY 再 ADD）→ `SET FOREIGN_KEY_CHECKS=1`。生产执行前在备份库验证，大表建议用 pt-online-schema-change 等在线迁移工具。
